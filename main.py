...
def check_bitget_signals():
    send_telegram_msg("🔍 *Bitget 策略掃描 (含止損重置邏輯)...*")
                ...
                for bar in custom_3h_list:
                    if entry_price is None:
                        ...
                        if bar['low'] <= stop_loss:
                            entry_price = None # 重置進場狀態，繼續尋找下一個進場點
                            stop_loss = None
                            target_p = None

                # 最終篩選：目前必須有進場點，且沒有達成 1:2 (因為達成後會 break)
                if entry_price and not is_completed:
                    clean_name = item['symbol'].split(':')[0]
                    hit_symbols.append(
                        f"• `{clean_name:10}` (進行中)\n"
                        f"  壓力: `{item['pressure_p']}` (`{item['pressure_d']}`)\n"
                        f"  最新進場: `{entry_price:.4f}` / 止損: `{stop_loss:.4f}`"
                    )
            except:
                continue
